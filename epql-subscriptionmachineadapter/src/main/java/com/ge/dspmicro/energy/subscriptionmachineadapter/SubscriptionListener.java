...
public class SubscriptionListener implements IDataSubscriptionListener {

	private static final Logger _logger = LoggerFactory.getLogger(SubscriptionListener.class);
	...
	public void onDataUpdate(ISubscriptionMachineAdapter sender, List<PDataValue> values) {
		...
		for (PDataValue value : values) {
			_logger.info("1...Internal Listener with Value:  " + value); //$NON-NLS-1$
		}
	}
	...
	public void onDataUpdate(ISubscriptionMachineAdapter sender, Map<String, String> properties, List<PDataValue> values) {
		...
		//for (PDataValue value : values) {
		//	_logger.info("2...Internal Listener with Value:  " + value); //$NON-NLS-1$
		//}
		_logger.info("2...Sending data of subscription :[" + properties.get(IDataSubscription.PROPKEY_SUBSCRIPTION) + "]");
	}
